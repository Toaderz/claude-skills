...
name: icm-architect
description: >
  Designs a process, body of knowledge, or codebase into an ICM workspace — folder
  structure as agent architecture, where numbered folders carry sequence, hierarchy
  carries context scope, and plain markdown files carry state — or restructures an
  existing folder, repo, or vault into one. Use when turning a recurring workflow into
  an agent-runnable pipeline, organizing scattered notes into a library one agent can
  walk, mapping a team or company as connected context, auditing a codebase into a
  walkable edit map of objects and change impact, or on "make this an ICM", "map this
  repo", "structure this for agents", "build me a workspace", "estructura este repo",
  "organiza esto para agentes". Do NOT use for one-off or non-repeating work, for a
  single throwaway script, or as a default applied to every new project — a workspace
  for something done twice is scaffolding, not architecture.
---
...
- [assets/templates/](assets/templates/) — copyable starters: `CLAUDE.md`, workspace `CONTEXT.md`, `stage-CONTEXT.md`, `object.md`, `process.md`, `schema.md`, `questionnaire.md`.
