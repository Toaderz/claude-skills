...
description: >
  Enforces disciplined Python development on projects with real logic: general fixes
  instead of hardcoded patches, root-cause tracing through the pipeline, stable
  interfaces, and a regression test before anything is called done. Use when building,
  debugging, refactoring, or extending a Python project with multiple modules, a data
  pipeline, or anything where robustness matters more than speed, and on "fix this bug",
  "add this feature", "refactor", "it's breaking", "doesn't work for this case", "write
  a parser/pipeline/detector", "arregla este bug", "no jala para este caso",
  "refactoriza esto". Do NOT use for one-off scripts, simple utilities, or questions
  about Python syntax.
---
...
For detecting subtle development anti-patterns that waste time. Watch for these while
working, and name them out loud the moment one appears — an unnamed anti-pattern keeps
running:
