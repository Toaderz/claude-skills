...
## Auto-use in this account

This skill is one of the always-on globals for this account (see the root `CLAUDE.md`): apply it by default whenever a new project or repository is being started, without waiting to be asked. Use Build mode to scaffold the workspace as work is described, or Restructure mode the first time substantive work touches an existing unstructured folder. Skip it only for genuinely one-off, non-repeating tasks (the guardrail above) — say so briefly rather than forcing a workspace onto a single throwaway script.
